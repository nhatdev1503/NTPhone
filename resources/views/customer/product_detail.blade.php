							...
							<div class="clearfix form-group">
								<div class="flex-quantity">
									<div class="custom custom-btn-number show">
										...
										<button class="btn_num num_1 button button_qty" type="button" onclick="decrementQuantity(); return false;">
...
<input type="text" id="qtym" name="quantity" value="1" maxlength="5" class="form-control prd_quantity"
       onkeypress="if (isNaN(this.value + String.fromCharCode(event.keyCode))) return false;"
       onchange="document.getElementById('selectedQuantity').value = this.value; updateValues();">

<button class="btn_num num_2 button button_qty" type="button" onclick="incrementQuantity(); return false;">
    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
        ...
        <path d="M8 4a.5.5 0 0 1 .5.5v3h3a.5.5 0 0 1 0 1h-3v3a.5.5 0 0 1-1 0v-3h-3a.5.5 0 0 1 0-1h3v-3A.5.5 0 0 1 8 4" />
									...
									<form id="productForm" method="POST">
										...
										<div class="group-button">
											<button type="button" class="btn-buyNow"
												data-action="{{ route('customer.buynow') }}"
												onclick="submitForm(this.dataset.action)">
												Mua ngay
											</button>
							...
							<script>
    // Hàm để tăng số lượng khi nhấn "+"
    function incrementQuantity() {
        var qtyInput = document.getElementById('qtym');
        var qty = parseInt(qtyInput.value);
        if (!isNaN(qty)) {
            qtyInput.value = qty + 1;  // Tăng giá trị lên 1
            console.log("Updated Quantity:", qtyInput.value);  // Kiểm tra giá trị
            updateValues();  // Cập nhật giá trị trong form
        }
    }

    // Hàm để giảm số lượng khi nhấn "-"
    function decrementQuantity() {
        var qtyInput = document.getElementById('qtym');
        var qty = parseInt(qtyInput.value);
        if (!isNaN(qty) && qty > 1) {
            qtyInput.value = qty - 1;  // Giảm giá trị đi 1, không để dưới 1
            console.log("Updated Quantity:", qtyInput.value);  // Kiểm tra giá trị
            updateValues();  // Cập nhật giá trị trong form
        }
    }

    // Hàm cập nhật các giá trị trong form trước khi gửi
    function updateValues() {
        var selectedStorage = jQuery('.storage-option.selected').data('storage');
        var selectedColor = jQuery('.color-selector:checked').val();
        var selectedQuantity = jQuery('#qtym').val();  // Đảm bảo lấy đúng giá trị từ #qtym
    ...
    // Hàm gửi form sau khi đã cập nhật giá trị
