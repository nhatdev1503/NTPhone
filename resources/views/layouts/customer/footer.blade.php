	...
	<div class="listSharing_action">
		...
		<ul class="addThis_listing">
			...
			<style>
				...
				.chat-header {
					background: linear-gradient(to right, #fecf72, #ef9f00);
					padding: 10px;
					color: white;
					font-weight: bold;
					display: flex;
			...
			<li class="addThis_item">
				<a class="addThis_item--icon" href="#" title="chat" aria-label="chat" onclick="toggleChatForm()">
					<svg width="44" height="44" viewBox="0 0 44 44" fill="none" xmlns="http://www.w3.org/2000/svg">
						<circle cx="22" cy="22" r="22" fill="url(#paint5_linear)"></circle>
						<path class="down"
							d="M22 10C17.0374 10 13 13.7367 13 18.3297C13 24.0297 21.0541 32.3978 21.397 32.7512C21.7191 33.0832 22.2815 33.0826 22.603 32.7512C22.9459 32.3978 31 24.0297 31 18.3297C30.9999 13.7367 26.9626 10 22 10ZM22 22.5206C19.5032 22.5206 17.4719 20.6406 17.4719 18.3297C17.4719 16.0188 19.5032 14.1388 22 14.1388C24.4968 14.1388 26.528 16.0189 26.528 18.3297C26.528 20.6406 24.4968 22.5206 22 22.5206Z"
					...
					<span class="tooltip-text">Trò chuyện trực tiếp</span>
				</a>
				<div id="chatForm" class="chat-box hidden">
					<div class="chat-header">
						<strong>💬 Chat với chúng tôi</strong>
						<button onclick="toggleChatForm()">✖</button>
					</div>
					<div class="chat-body">
...
<script>
	...
	function innerViewChatClient(data) {
		let view = document.getElementById('messages');
		let chatHTML = '';
			...
			if (message.sender_id === currentUserId) {
				...
				<div class="chat-list right">
					<div class="user-chat-content">
						<p>${message.message}</p>
						<p class="meta"><time datetime="2018">${formatDateTime(message.created_at)}</time></p>
					</div>
				</div>`;
			} else {
				chatHTML += `
		...
		if (data.sender.id === currentUserId) {
			chatHTML += `
				<div class="chat-list right">
					<div class="user-chat-content">
						<p>${data.message}</p>
						<p class="meta"><time datetime="2018">${formatDateTime(new Date())}</time></p>
					</div>
				</div>`;
		} else {
			chatHTML += `
	...
	function sendMessage(event) {
		...
		var route = '{{ route('chatClient.sendMessage') }}';
